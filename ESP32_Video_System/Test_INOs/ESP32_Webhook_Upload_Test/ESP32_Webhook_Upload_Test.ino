...
const char* webhook_url = "https://webhook.site/1d32cf06-67c4-41f7-b24c-7141b7da18cd";
...
void setup() {
  ...
  // --- 7. THE MAIL TRUCK (Upload via HTTP POST) ---
  File uploadFile = SD_MMC.open("/test_video.avi", FILE_READ);
  if (!uploadFile) {
    Serial.println("ERROR: Could not open file for uploading.");
    return;
  }

  Serial.println("Starting HTTP POST Delivery...");
  
  HTTPClient http;
  http.begin(webhook_url);
  http.addHeader("Content-Type", "video/x-msvideo");
  
  // The ESP32 HTTPClient can read directly from the SD card stream
  int httpResponseCode = http.sendRequest("POST", &uploadFile, uploadFile.size());
  
  if (httpResponseCode > 0) {
    Serial.printf("HTTP Response code: %d\n", httpResponseCode);
    Serial.println("SUCCESS: Video delivered to Webhook!");
  } else {
